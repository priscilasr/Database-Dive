DROP TABLE NotRecomended;
CREATE TABLE IF NOT EXISTS NotRecomended (id, titleId, add_date, rating, genres, num_gen, actors, num_act, directors, num_dir, writers, num_wri) AS(
...
ORDER BY add_date, rating, num_gen DESC, num_act DESC, num_wri DESC
);
DROP TABLE NRF;
CREATE TABLE NRF AS SELECT * FROM NotRecomended; 
DELETE FROM NRF y
    WHERE EXISTS (SELECT 1 FROM NRF y2 WHERE y.titleId = y2.titleId AND (y2.id > y.id));